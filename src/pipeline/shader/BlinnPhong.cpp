...
BPShader_GlobalNormalMap::BPShader_GlobalNormalMap(const Model& _model, const Camera& _camera)
    : model(_model), camera(_camera) 
{ 
    light = {0, 2, 2, 1};       // 默认光源位置
    light_color = white;        // 默认光源颜色
...
vec4
BPShader_GlobalNormalMap::vertex(const face_obj& f, int idx)
{ 
    //                                   |  这里用std::get<>方法来提取tuple，第0项是vertex索引
    //                                  \|/
    vec4 v = MV * model.getVertex(std::get<0>(f[idx])); // 从model的v中获取原始vertex，进行MV变换；此时镜头位于原点，并且世界还没有变形
...
std::pair<bool, TGAColor> 
BPShader_GlobalNormalMap::fragment(const vec3_f abg) const 
{
    /*
        这里插值同样要注意修正投影空间的畸变
        因为重心坐标来自投影空间，不能够直接套在相机空间内
